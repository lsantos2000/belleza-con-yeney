...
PAGES=ROOT/'resources/images/books/ElMetodoCabelloSaludable/es/video-pages'
AUDIO=ROOT/'resources/audio/ElMetodoCabelloSaludable/narracion.m4a'
VIDEO=ROOT/'resources/videos/ElMetodoCabelloSaludable'
DOCS=ROOT/'resources/docs/ElMetodoCabelloSaludable'
STORYBOARD_IMAGE=ROOT/'resources/images/books/ElMetodoCabelloSaludable/es/storyboard.jpg'
WORK=HERE/'build'
FF=imageio_ffmpeg.get_ffmpeg_exe()
COLOR='#7C293D';CREAM='#FFF7ED';GOLD='#D7AE68'
OUT=VIDEO/'el-metodo-cabello-saludable-promo-3-minutos.mp4'
...
def frame(sec,s,n):
    ...
    if s['kind']=='cta':
        p=ImageOps.fit(Image.open(AUTHOR/Path(s['photo']).name).convert('RGB'),(468,565),Image.Resampling.LANCZOS,centering=(.5,.38));im.paste(p,(16,88))
        ...
        for i,page in enumerate(s['pages']):
            contain(im,PAGES/f'pagina-{page:02}.jpeg',(36+i*width,98,width-14,545))
    else:
        ...
        if 'photo' in s:
            contain(im,AUTHOR/Path(s['photo']).name,(x+13,y+9,w-26,h-18))
        contain(im,PAGES/f"pagina-{s['page']:02}.jpeg",(16 if right else 770,88,494,565))
...
def validate(sections):
    scenes=[s for sec in sections for s in sec['scenes']];paths=[AUTHOR/Path(s['photo']).name for s in scenes if 'photo' in s]
    ...
    DOCS.mkdir(parents=True,exist_ok=True);(DOCS/'verification.json').write_text(json.dumps(report,indent=2),encoding='utf-8');return report
def main():
    ...
    STORYBOARD_IMAGE.parent.mkdir(parents=True,exist_ok=True);sheet.save(STORYBOARD_IMAGE,quality=94)
